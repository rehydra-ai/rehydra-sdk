...
export class NodeStorageProvider implements StorageProvider {
  ...
  /**
   * Extracts a ZIP file using fflate or system commands
   */
  async extractZip(zipData: Uint8Array, destDir: string): Promise<string[]> {
    // Try using fflate first (pure JS, works everywhere)
    try {
      ...
      return this.extractZipWithCommand(zipData, destDir);
    }
  }

  /**
   * Fallback ZIP extraction using system commands
   */
  private async extractZipWithCommand(
    zipData: Uint8Array,
    destDir: string
  ): Promise<string[]> {
    ...
    const tempZipPath = nodePath.join(destDir, "_temp.zip");
    await this.writeFile(tempZipPath, zipData);

    try {
      // Try unzip command
      await execAsync(`unzip -o "${tempZipPath}" -d "${destDir}"`);
    } catch {
      // Try tar as fallback
      try {
        await execAsync(`tar -xf "${tempZipPath}" -C "${destDir}"`);
      } catch {
        throw new Error(
          "Could not extract ZIP file. Please install 'unzip' command or ensure fflate is available."
        );
      }
    }
    ...
    return entries
      .filter((e) => e !== "_temp.zip")
      .map((e) => nodePath.join(destDir, e));
  }
}
